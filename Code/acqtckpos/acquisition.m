...
if file.dataPrecision == 2  % 16bits
    rawsignal = fread(file.fid,signal.Sample*file.dataType*10,'int16')'; % 10ms data for acquisition
    ...
    rawsignal = fread(file.fid,signal.Sample*file.dataType*10,'int8')'; % 10ms data for acquisition
 if file.dataType == 2  % I/Q 
        I_signal = rawsignal(1:2:end); 
        Q_signal = rawsignal(2:2:end);  
        rawsignal = I_signal + 1i * Q_signal;  
    end
end
...
for svindex = 1:32
    ...
    fprintf('SNR = %2.2f\n', SNR);
    % Store SNR for all satellites regardless of acquisition status
    Acquired.allSNR(svindex) = SNR;
    if SNR >= 18  % acquisition thredhold
        ...
        Acquired.correlation{svindex} = correlation;
    end
end
plot_snr_acquisition(Acquired, 32);
...
if file.dataPrecision == 2
    ...
    rawdata = fread(file.fid, signal.Sample * file.dataType * (acq.L+1), 'int8')';
    if file.dataType == 2  % I/Q 数据
        I_signal = rawdata(1:2:end);
        Q_signal = rawdata(2:2:end);
        longrawsignal = I_signal + 1i * Q_signal;
    else
        longrawsignal = rawdata;
    end
end
...
for svindex = 1 : length(Acquired.sv)     
    ...
    fftSignal = abs(fftshift(fft(CarrSignal, fftlength))); 
    ...
    %AcquisitionPlot(acq,Acquired.sv(svindex),Acquired,signal);
    fprintf('SV[%2d] SNR = %2.2f Code phase = %5d  Doppler = %5f  Doppler frequency = %5f\n', ...
